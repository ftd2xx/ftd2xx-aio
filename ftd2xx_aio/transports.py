        ...
        if self._has_reader and not self._closing:
            ...
            try:
                ...
                if self._modem != modem:
                    self._protocol.modem_received(modem, modem ^ self._modem)
                    self._modem = modem
            except DeviceError as exc:
                self._fatal_error(exc, "Fatal error on ftd2xx transport")
